...
class ArrowPairedDataset(Dataset):
    """Read paired clean/noisy utterances from a HuggingFace Arrow stream.

    The expected Arrow fields are ``audio_path`` (clean), ``cond_audio_path``
    (noisy), ``text`` (a character list, possibly empty), and ``duration``.
    Waveform preprocessing intentionally follows the original FlowSE loader:
    read/resample to 16 kHz, resample to 24 kHz, then compute the Vocos mel.
    """
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "ArrowPairedDataset requires pyarrow. Install it in the FlowSE environment."
            ) from exc

        records = []
        with open(arrow_path, "rb") as file:
            reader = ipc.open_stream(file)
            required = {"audio_path", "cond_audio_path", "text", "duration"}
            missing = required.difference(reader.schema.names)
            if missing:
                raise ValueError(
                    f"{arrow_path} is missing Arrow fields: {sorted(missing)}"
                )
            for batch in reader:
                clean_paths = batch.column("audio_path")
                noisy_paths = batch.column("cond_audio_path")
                texts = batch.column("text")
                durations = batch.column("duration")
                for index in range(batch.num_rows):
                    raw_text = texts[index].as_py()
                    if isinstance(raw_text, list):
                        raw_text = "".join(raw_text)
                    records.append(
                        {
                            "clean_path": clean_paths[index].as_py(),
                            "noisy_path": noisy_paths[index].as_py(),
                            "text": raw_text or "",
                            "duration": float(durations[index].as_py()),
                        }
                    )
        ...
        if split != "all":
            if not 0.0 < val_ratio < 1.0:
                raise ValueError("val_ratio must be between 0 and 1 for train/val splits")
    ...
    def get_frame_len(self, index):
        return self.records[index]["duration"] * 24000 / 256

    def __getitem__(self, index):
        record = self.records[index]
        clean = get_firstchannel_read(record["clean_path"], fs=self.sample_rate)
        noisy = get_firstchannel_read(record["noisy_path"], fs=self.sample_rate)

        # Keep the paired signals aligned if their file lengths differ slightly.
        length = min(clean.shape[-1], noisy.shape[-1])
        clean = clean[:length]
        noisy = noisy[:length]

        clean = self.resampler(clean.unsqueeze(0))
        noisy = self.resampler(noisy.unsqueeze(0))
        clean_mel = self.mel_spectrogram(clean).squeeze(0)
        noisy_mel = self.mel_spectrogram(noisy).squeeze(0)

        text_chars = pinyin(record["text"], style=Style.TONE3)
        text = ["".join(item) for item in text_chars]
        return {
            "label_mel_spec": clean_mel,
            "noisy_mel_spec": noisy_mel,
            "label_path": record["clean_path"],
            "text": text,
        }
